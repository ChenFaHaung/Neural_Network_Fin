...
public class Mperceptron {
	...
	public double cal(double learnTemp, Perceptron[][] temp, ArrayList<Double> ex) {
		int count = 1;
		int ctemp = 2;
		while (ctemp < ex.size()) {
			count++;
		...
		for (int i = 0; i < p[temp.length - 1].length; i++) {
			p[temp.length - 1][i].randomWeight(dataTrain[0].length - 2);
		}
	...
	public double train(double[] input, int layer, double[] expected) {
		...
		if (layer == 0) {
			for (int i = 0; i < p[layer].length; i++) {
				eTemp = 0;
				eTemp = expected[i] - p[layer][i].output;//expected[i+1]
